...
export default function WidgetPage({ params }: { params: Promise<{ code: string }> }) {
  ...
  const fetchGreeting = async () => {
    ...
    try {
      ...
      setMessages([{ role: 'assistant', content: 'Trợ lý đang gặp lỗi kết nối. Vui lòng kiểm tra internet và thử lại.' }]);
    }
  };
  ...
  useEffect(() => {
    ...
    // Thử lại khi có mạng trở lại
    const handleOnline = () => {
      if (messages.length <= 1 && messages[0]?.content.includes('Đang kết nối')) {
        fetchGreeting();
      }
    };
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: `[Lỗi kết nối]: Vui lòng kiểm tra lại mạng của bạn.` }]);
    } finally {
      setLoading(false);
    }
  };
