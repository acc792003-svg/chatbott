...
export async function sendFacebookAction(sender_id: string, page_access_token: string, action: 'mark_seen' | 'typing_on' | 'typing_off') {
  try {
    const response = await fetch(`https://graph.facebook.com/v12.0/me/messages?access_token=${page_access_token}`, {
      method: 'POST',
      headers: {
    ...
    const data = await response.json();
    if (data.error) {
      console.error(`❌ Facebook Action API Error (${action}):`, data.error);
      return false;
    }
    
    return true;
  } catch (e) {
    console.error(`❌ Network error sending FB action (${action}):`, e);
    return false;
  }
}
